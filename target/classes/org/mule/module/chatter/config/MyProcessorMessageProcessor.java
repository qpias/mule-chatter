...
/**
 * MyProcessorMessageProcessor invokes the {@link org.mule.module.chatter.ChatterModule#myProcessor(java.util.List)} method in {@link ChatterModule }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private Object messageSegments;
    private List<Map<String, String>> _messageSegmentsType;
    private static Logger logger = LoggerFactory.getLogger(MyProcessorMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws InitialisationException
    {
        ...
        if (moduleObject == null) {
            try {
                moduleObject = muleContext.getRegistry().lookupObject(ChatterModuleLifecycleAdapter.class);
                if (moduleObject == null) {
                    throw new InitialisationException(MessageFactory.createStaticMessage("Cannot find object"), this);
                }
            } catch (RegistrationException e) {
                throw new InitialisationException(CoreMessages.initialisationFailure("org.mule.module.chatter.config.ChatterModuleLifecycleAdapter"), e, this);
            }
        }
    }
     ...
     * @param value Value to set
     */
    public void setMessageSegments(Object value) {
        this.messageSegments = value;
    }
        ...
        throws MuleException
    {
        ...
        ChatterModuleLifecycleAdapter castedModuleObject = null;
        if (moduleObject instanceof String) {
            castedModuleObject = ((ChatterModuleLifecycleAdapter) muleContext.getRegistry().lookupObject(((String) moduleObject)));
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("myProcessor"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
            castedModuleObject = ((ChatterModuleLifecycleAdapter) moduleObject);
        }
        try {
            List<Map<String, String>> transformedMessageSegments = ((List<Map<String, String>> ) evaluateAndTransform(muleMessage, MyProcessorMessageProcessor.class.getDeclaredField("_messageSegmentsType").getGenericType(), messageSegments));
            retryCount.getAndIncrement();
            Object resultPayload;
            resultPayload = castedModuleObject.myProcessor(transformedMessageSegments);
